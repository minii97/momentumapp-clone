todoHeaderMoreButton.addEventListener('click', (e) => {
...
function actionForTodoHeaderMoreItem(item) {
  ...
  if (existTodo.classList.contains('is-active')) {
    ...
    if (
      item.classList.contains('delete-all-btn') ||
      item.classList.contains('check-all-btn') ||
      item.classList.contains('clear-all-btn')
    ) {
      activeTodoInput()
...
todoHeaderMoreMenuItemButtons.forEach((item) => {
  item.addEventListener('click', () => {
    actionForTodoHeaderMoreItem(item)
  })
})
